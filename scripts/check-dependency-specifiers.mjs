	...
	for (const section of dependencySections) {
		...
		for (const [name, specifier] of Object.entries(dependencies)) {
			if (typeof specifier !== "string") {
				failures.push(`${file}: ${section}.${name} must be a string version specifier`);
				continue;
			}
			if (isNonRegistrySpecifier(specifier)) {
				failures.push(`${file}: ${section}.${name} must use a registry version, found ${specifier}`);
				continue;
			}
			if (!exactVersionPattern.test(getVersionSpecifier(specifier))) {
				failures.push(`${file}: ${section}.${name} must be pinned to an exact version, found ${specifier}`);
			}
		}
	}
}
...
if (failures.length > 0) {
	console.error("Direct dependencies must use exact registry versions:");
