...
const allowValue = process.env.SKYLARK_ALLOW_LOCKFILE_CHANGE;
...
if (allowed) {
	console.error("package-lock.json is staged; SKYLARK_ALLOW_LOCKFILE_CHANGE is set, allowing commit.");
	process.exit(0);
}
...
console.error("  - confirm every new or updated package is intentional");
console.error("  - confirm direct dependencies stay pinned to exact registry versions");
console.error("  - review any new lifecycle scripts in the dependency tree");
console.error("  - confirm no file/link/workspace dependency was introduced");
...
console.error("  SKYLARK_ALLOW_LOCKFILE_CHANGE=1 git commit ...");
process.exit(1);
