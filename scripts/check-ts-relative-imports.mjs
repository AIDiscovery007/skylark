...
const ignoredDirectories = new Set([".git", ".husky", "coverage", "dist", "node_modules", "out"]);
const files = [];
...
for (const file of files.sort()) {
	...
	function checkSpecifier(node) {
		if (!isRelativeJavaScriptSpecifier(node.text)) {
			return;
		}
...
if (failures.length > 0) {
	console.error("Relative .js imports are not allowed in Skylark TypeScript files:");
